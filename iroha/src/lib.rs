...
pub struct Iroha {
    ...
    kura: Shared<Kura>,
    transactions_receiver: Shared<TransactionReceiver>,
    wsv_blocks_receiver: Shared<CommittedBlockReceiver>,
    kura_blocks_receiver: Shared<ValidBlockReceiver>,
    sumeragi_message_receiver: Shared<SumeragiMessageReceiver>,
    block_sync_message_receiver: Shared<BlockSyncMessageReceiver>,
    world_state_view: Shared<WorldStateView>,
    block_sync: Shared<BlockSynchronizer>,
}
...
impl Iroha {
    ...
    pub fn new(config: Configuration) -> Self {
        ...
        let world_state_view = Shared::new(WorldStateView::new(Peer::with_domains(
        ...
        let kura = Shared::new(kura);
        let block_sync = Shared::new(BlockSynchronizer::from_configuration(
            &config.block_sync_configuration,
            kura.clone(),
            sumeragi.clone(),
            PeerId::new(&config.torii_configuration.torii_url, &config.public_key),
        ));
        let queue = Shared::new(Queue::from_configuration(
            &config.queue_configuration,
        ));
        Iroha {
            queue,
            torii: Shared::new(torii),
            sumeragi,
            kura,
            world_state_view,
            transactions_receiver: Shared::new(transactions_receiver),
            wsv_blocks_receiver: Shared::new(wsv_blocks_receiver),
            sumeragi_message_receiver: Shared::new(sumeragi_message_receiver),
            kura_blocks_receiver: Shared::new(kura_blocks_receiver),
            block_sync_message_receiver: Shared::new(block_sync_message_receiver),
            block_sync,
        }
    }
    ...
    pub async fn start(&self) -> Result<(), String> {
        ...
        let torii = Shared::clone(&self.torii);
        ...
        let transactions_receiver = Shared::clone(&self.transactions_receiver);
        let queue = Shared::clone(&self.queue);
        ...
        let queue = Shared::clone(&self.queue);
        let sumeragi = Shared::clone(&self.sumeragi);
        ...
        let wsv_blocks_receiver = Shared::clone(&self.wsv_blocks_receiver);
        let world_state_view = Shared::clone(&self.world_state_view);
        let sumeragi = Shared::clone(&self.sumeragi);
        let block_sync = Shared::clone(&self.block_sync);
        ...
        let sumeragi_message_receiver = Shared::clone(&self.sumeragi_message_receiver);
        let sumeragi = Shared::clone(&self.sumeragi);
        ...
        let block_sync_message_receiver = Shared::clone(&self.block_sync_message_receiver);
        let block_sync = Shared::clone(&self.block_sync);
        ...
        let kura_blocks_receiver = Shared::clone(&self.kura_blocks_receiver);
        let kura = Shared::clone(&self.kura);
